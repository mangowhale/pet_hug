	...
	<jsp:include page="/WEB-INF/views/module/nav.jsp" flush="false"/>
        ...
        <section>
            <div class="jumbotron background-lightgreen pt-5 pb-5">
                <div class="container">
                    <h2 class="display-3 font text-center">식음료</h2>
                </div>    
            </div>
        </section>
   	...
   	<div class="tap_bar">
	    ...
	    <a href="/pet_hug/cafes" class="w-btn-outline mypage-select">식음료</a>
            ...
            <div class="container">
                <div class="label-w10">
                    ...
                    function navigateToAll() {
                        window.location.href = '/pet_hug/cafes';
				    ...
				    function updateURL(areaName) {
				        var url = "/pet_hug/cafes/areaName/" + encodeURIComponent(areaName);
					...
					<script>
						// 페이지 이동 시 스타일 초기화 /pet_hug/cafes~~~~ 외 페이지로
        ...
        <section class="py-5 my-3" style="min-height: 900px;">
            <div class="container">
                ...
                <div class="w-100 mb-5">
			        ...
			        <form id="searchForm" class="text-center" action="/pet_hug/cafes/" method="GET" onsubmit="updateAction(event)">
			    ...
			    <div style="float: right; transform:translate(0px,-85px);">
					<c:if test="${sessionScope.sessionId == 'admin'}"> <!-- 특정 ID로 로그인 한 경우에만 아래 버튼 표시 -->
						<a href="/pet_hug/cafes/add_api" class="w-btn-outline w-btn-green2-outline" >api업데이트</a>
						...
						<div class="d-flex flex-4 flex-wrap w-100">
			                <c:forEach items="${cafeList}" var="cafe">
			                    ...
			                    <div class="border p-3 mb-3 mr-3" style="height: 350px">
			                        <a href="<c:url value="/cafes/cafe?num=${cafe.contentSeq}"/>" role="button"><!--개별 페이지 이동 주소 링크-->
			                            <div class="mb-3">
			                                <!-- 사진 -->
			                                <img src="${cafe.p1}" alt="" class="img-fluid">
			                            </div>              
			                            <div>
			                                <h4 class="fw-bold color-black" style="font-size: 1rem; height: 2rem">${cafe.title}</h4>
			                                <div class="mb-3">
			<%-- 							    	<div class="star_rating mb-4">
										        	<c:forEach begin="1" end="5" var="i">
										            	<span class="star ${i <= cafe.star_avg ? 'on' : ''}" value="${i}"></span>
										            </c:forEach>
										            <span style="color:black; margin-left: 10px; color: gray">(${cafe.review_num})</span>
										         </div> --%>
			                                <p class="mb-1 color-black"><i class="fa-solid fa-phone col-sm-1"></i> ${cafe.tel}</p>
			                                <p class="color-black"><i class="fas fa-map-marker-alt col-sm-1"></i> ${cafe.address}</p>
			                                </div>
			                            </div>
			                        </a>                     
			                    </div> 
                ...
                <script>
				    document.addEventListener('DOMContentLoaded', function() {
				        var cafeContainer = document.getElementById('cafeContainer');
				        var cafes = document.querySelectorAll('.border.p-3.mb-3');
				        ...
				        function updatePage() {
				            // 현재 페이지에 표시할 요소 선택
				            var currentPageCafes = Array.from(cafes).slice(startIndex, endIndex);
				            ...
				            // 현재 페이지에 해당하는 요소 보이기
				            currentPageCafes.forEach(function(cafe) {
				                cafe.style.display = 'block';
				            });
				            
				         	// 총 페이지 수 계산
				            var totalPages = Math.ceil(cafes.length / itemsPerPage);
				            ...
				            document.getElementById('nextPageButton').disabled = endIndex >= cafes.length;
				            document.getElementById('lastPageButton').disabled = endIndex >= cafes.length;
				        }
				        ...
				        function prevPage() {
				            if (currentPage > 1) {
				                ...
				                endIndex = Math.min(currentPage * itemsPerPage, cafes.length);
				                updatePage();
				            }
				        }
				        ...
				        document.getElementById('lastPageButton').addEventListener('click', function() {
				            currentPage = Math.ceil(cafes.length / itemsPerPage);
				            startIndex = (currentPage - 1) * itemsPerPage;
				            endIndex = cafes.length;
				            updatePage();
				        });
				    });
				</script>
