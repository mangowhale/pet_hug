<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import = "com.springmvc.repository.CourseRepositoryImpl" %>
...
<body>
	...
	<div class="container">
		<h2 class="fw-bold pt-5 pb-4 contentTitle">${spot.title}</h2>
    	<div>
        	<div class="d-flex">
	            <img src="${spot.p1}" class="slide"/>
	            <c:if test="${spot.p2 ne null}">
	            	<img src="${spot.p2}" class="slide"/>
	            </c:if>
	            <c:if test="${spot.p3 ne null}">
	            	<img src="${spot.p3}" class="slide"/>
	            </c:if>
	            <c:if test="${spot.p4 ne null}">
	            	<img src="${spot.p4}" class="slide"/>
	            </c:if>
        	</div>
         <hr>
         <div class="py-3">
			<div class="infoContainer d-flex">
				<p><i class="fa-solid fa-location-dot p-2"></i><b>주소</b> : ${spot.address}</p>
                <p><i class="fa-solid fa-phone p-2"></i><b>연락처</b> : ${spot.tel}</p>
			</div>	
			<div class="infoContainer d-flex">
				<p><i class="far fa-clock p-2"></i><b>이용시간</b> : ${spot.usedTime}</p>
                <p><i class="fa-solid fa-hashtag p-2"></i><b>키워드</b> : ${spot.keyword}</p>
			</div>
			
			<div class="pt-3 pb-5 d-flex">
				<c:if test="${spot.bathFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-bath"></i>
						<p>반려견 목욕</p>
					</div>
				</c:if>
				<c:if test="${spot.provisionFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-soap"></i>
						<p>비품 제공</p>
					</div>
				</c:if>
				<c:if test="${spot.petFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-bone"></i>
						<p>펫 동반식당</p>
					</div>
				</c:if>
				<c:if test="${spot.petWeight ne null && not empty spot.petWeight}">
					<div class="miniTag weightConstraint">
						<h6><span>${spot.petWeight}</span>kg</h6>
						<p>몸무게 제한</p>
					</div>
				</c:if>
				<c:if test="${empty spot.petWeight}">
					<div class="miniTag weightConstraint">
						<i class="fas fa-times"></i>
						<p>몸무게 제한</p>
					</div>
				</c:if>
				<c:if test="${spot.emergencyFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-hand-holding-medical"></i>
						<p>응급 수칙</p>
					</div>
				</c:if>
				<c:if test="${spot.entranceFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-coins"></i>
						<p>입장료 있음</p>
					</div>
				</c:if>
				<c:if test="${spot.parkingFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-car"></i>
						<p>주차장</p>
					</div>
				</c:if>
				<c:if test="${spot.inOutFlag eq 'OUT'}">
					<div class="miniTag">
						<i class="fas fa-tree"></i>
						<p>야외</p>
					</div>
				</c:if>
				<c:if test="${spot.inOutFlag eq 'IN'}">
					<div class="miniTag">
                ...
                <div class="col-md-12">
                    <form:form name="addForm" method="put">
                    	<c:if test="${spot.homePage ne null }">
               				<a href="${spot.homePage}" target="_blank" class="homepageLink w-btn-outline1"><i class="fas fa-globe"></i> 홈페이지 이동</a>
            			</c:if>            			
                        <a href="/pet_hug/spots" class="w-btn-outline1 w-btn-gray-outline">목록으로</a>
                        <c:if test="${sessionScope.sessionId == 'admin'}">
		                    <!-- 특정 ID로 로그인 한 경우에만 아래 두 버튼들 표시 -->
		                    <a href="<c:url value="/hotels/update?num=${spot.contentSeq}" />" class="btn btn-success">수정</a>
		                    <a href="<c:url value="javascript:deleteConfirm('${spot.contentSeq}')" />" class="btn btn-danger"><i class="far fa-trash-alt me-1"></i>삭제</a>
		                </c:if>
                    </form:form>
                </div>
                <br>
                <script>
                    function deleteConfirm(num) {
                        if (confirm("삭제합니다") == true) location.href = "/pet_hug/spots/delete?num=" + num;
                        else return;
                    }
                </script>
            </div>
                ...
                geocoder.addressSearch('${spot.address}', function(result, status) {
                    ...
                    if (status === kakao.maps.services.Status.OK) {
                        ...
                        var infowindow = new kakao.maps.InfoWindow({
                            content: '<div style="width:150px;text-align:center;padding:6px 0;">${spot.title}</div>'
                        });
