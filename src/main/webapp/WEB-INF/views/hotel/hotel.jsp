...
<link rel="stylesheet" href="${pageContext.request.contextPath}/resources/css/imageSlide.css?=ver3" type="text/css">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.1/dist/css/bootstrap.min.css" rel="stylesheet">
<title>숙소</title>
</head>
<body onload="arrangeSlides()">
   ...
   <section>
	...
	<div class="container">
		<h2 class="fw-bold pt-5 pb-4 contentTitle">${hotel.title}</h2>
    	<div>
        	<div class="d-flex">
	            <img src="${hotel.p1}" class="slide"/>
	            <c:if test="${hotel.p2 ne null}">
	            	<img src="${hotel.p2}" class="slide"/>
	            </c:if>
	            <c:if test="${hotel.p3 ne null}">
	            	<img src="${hotel.p3}" class="slide"/>
	            </c:if>
	            <c:if test="${hotel.p4 ne null}">
	            	<img src="${hotel.p4}" class="slide"/>
	            </c:if>
        	</div>
        <hr>
        <div class="py-3">
			<div class="infoContainer d-flex">
				<p><i class="fa-solid fa-location-dot p-2"></i><b>주소</b> : ${hotel.address}</p>
                <p><i class="fa-solid fa-phone p-2"></i><b>연락처</b> : ${hotel.tel}</p>
			</div>	
			<div class="infoContainer d-flex">
				<p><i class="far fa-clock p-2"></i><b>이용시간</b> : ${hotel.usedTime}</p>
                <p><i class="fa-solid fa-hashtag p-2"></i><b>키워드</b> : ${hotel.keyword}</p>
			</div>
			
			<div class="pt-3 pb-5 d-flex">
				<c:if test="${hotel.bathFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-bath"></i>
						<p>반려견 목욕</p>
					</div>
				</c:if>
				<c:if test="${hotel.provisionFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-soap"></i>
						<p>비품 제공</p>
					</div>
				</c:if>
				<c:if test="${hotel.petFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-bone"></i>
						<p>펫 동반식당</p>
					</div>
				</c:if>
				<c:if test="${hotel.petWeight ne null && not empty hotel.petWeight}">
					<div class="miniTag weightConstraint">
						<h6><span>${hotel.petWeight}</span>kg</h6>
						<p>몸무게 제한</p>
					</div>
				</c:if>
				<c:if test="${empty hotel.petWeight}">
					<div class="miniTag weightConstraint">
						<i class="fas fa-times"></i>
						<p>몸무게 제한</p>
					</div>
				</c:if>
				<c:if test="${hotel.emergencyFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-hand-holding-medical"></i>
						<p>응급 수칙</p>
					</div>
				</c:if>
				<c:if test="${hotel.entranceFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-coins"></i>
						<p>입장료 있음</p>
					</div>
				</c:if>
				<c:if test="${hotel.parkingFlag eq 'Y'}">
					<div class="miniTag">
						<i class="fas fa-car"></i>
						<p>주차장</p>
					</div>
				</c:if>
				<c:if test="${hotel.inOutFlag eq 'OUT'}">
					<div class="miniTag">
						<i class="fas fa-tree"></i>
						<p>야외</p>
					</div>
				</c:if>
				<c:if test="${hotel.inOutFlag eq 'IN'}">
					<div class="miniTag">
                ...
                <div class="col-md-12 py-3">
                    ...
                    <c:if test="${hotel.homePage ne null }">
               			<a href="${hotel.homePage}" target="_blank" class="homepageLink w-btn-outline1"><i class="fas fa-globe"></i> 홈페이지 이동</a>
                	</c:if>                           
                  	<c:if test="${not empty sessionScope.sessionId}">
                  		<a href="javascript:addToCart('/pet_hug/hotelcart/add/${hotel.contentSeq}')" class="w-btn-outline1 w-btn-green2-outline">관심등록</a>
						<a href="/pet_hug/hotelcart" class="w-btn-outline1 w-btn-yellow-outline">관심목록</a>																								
						<a href="/pet_hug/reserves/add?title=${hotel.title}&contentSeq=${hotel.contentSeq}" class="w-btn-outline1 w-btn-green2-outline">예약</a>
					</c:if>				  
		         	<a href="/pet_hug/hotels" class="w-btn-outline1 w-btn-gray-outline">숙소목록</a> 
		         	<c:if test="${sessionScope.sessionId == 'admin'}"> <!-- 특정 ID로 로그인 한 경우에만 아래 두 버튼들 표시 -->
		               	<a href="<c:url value="/hotels/update?num=${hotel.contentSeq}"/>" class="btn btn-success">수정</a>
		               	<a href="<c:url value="javascript:deleteConfirm('${hotel.contentSeq}')"/>" class="btn btn-danger"><i class="far fa-trash-alt me-1"></i>삭제</a>           
		         	</c:if>              					
					<script>
      ...
      <div id="지도넣을곳" class="py-3">
         ...
         <script>
	         ...
	         // 주소로 좌표를 검색합니다
	         geocoder.addressSearch('${hotel.address}', function(result, status) {
	             ...
	             // 정상적으로 검색이 완료됐으면 
	              if (status === kakao.maps.services.Status.OK) {
	                 ...
	                 var infowindow = new kakao.maps.InfoWindow({
	                     content: '<div style="width:150px;text-align:center;padding:6px 0;">${hotel.title}</div>'
	                 });
