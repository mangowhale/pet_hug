	...
	<jsp:include page="/WEB-INF/views/module/nav.jsp" flush="false"/>
        ...
        <section>
            <div class="jumbotron background-lightgreen pt-5 pb-5">
                <div class="container">
                    <h2 class="display-3 font text-center">숙소</h2>
                </div>    
            </div>
        </section>

   	<div class="tap_bar">
	    <a href="/pet_hug/hotels" class="w-btn-outline mypage-select">숙　소</a>
	    <a href="/pet_hug/spots" class="w-btn-outline w-btn-green2-outline">관광지</a>
        ...
        <section class="pt-5">
            <div class="container">
                ...
                <input type="radio" name="gangwon" id="강릉시">
                <input type="radio" name="gangwon" id="고성군">
                <input type="radio" name="gangwon" id="동해시">
                <input type="radio" name="gangwon" id="삼척시">
                <input type="radio" name="gangwon" id="속초시">
                <input type="radio" name="gangwon" id="양구군">
                <input type="radio" name="gangwon" id="양양군">
                <input type="radio" name="gangwon" id="영월군">
                <input type="radio" name="gangwon" id="원주시">
                <input type="radio" name="gangwon" id="인제군">
                <input type="radio" name="gangwon" id="정선군">
                <input type="radio" name="gangwon" id="철원군">
                <input type="radio" name="gangwon" id="춘천시">
                <input type="radio" name="gangwon" id="태백시">
                <input type="radio" name="gangwon" id="평창군">
                <input type="radio" name="gangwon" id="홍천군">
                <input type="radio" name="gangwon" id="화천군">
                <input type="radio" name="gangwon" id="횡성군"> -->
                <div class="label-w10">
                    ...
                    <!-- <script>
					    function hotels() 
					    {
					        window.location.href = "/pet_hug/hotels";
					    }
					</script> -->
					...
					<!-- <script>
					    ...
					    {
					        var url = "/pet_hug/hotels/areaName/" + encodeURIComponent(areaName);
					...
					<script>
						// 페이지 이동 시 스타일 초기화 /pet_hug/hotels~~~~ 외 페이지로
        ...
        <section class="py-5 my-3" style="min-height: 900px;">
            <div class="container">
                ...
                <div class="w-100 mb-5">
			        ...
			        <form id="searchForm" class="text-center" action="/pet_hug/hotels/" method="GET" onsubmit="updateAction(event)">
			    ...
			    <div style="float: right; transform:translate(0px,-85px);">
					<c:if test="${sessionScope.sessionId == 'admin'}"> <!-- 특정 ID로 로그인 한 경우에만 아래 버튼 표시 -->
						<a href="/pet_hug/hotels/add_api" class="w-btn-outline w-btn-green2-outline" >api업데이트</a>
						...
						<div class="d-flex flex-4 flex-wrap w-100">
			                <c:forEach items="${hotelList}" var="hotel">
			                    ...
			                    <div class="border p-3 mb-3 mr-3" style="height: 350px">
			                        <a href="<c:url value="/hotels/hotel?num=${hotel.contentSeq}"/>" role="button"><!--개별 페이지 이동 주소 링크-->
			                            <div class="mb-3">
			                                <!-- 사진 -->
			                                <img src="${hotel.p1}" alt="" class="img-fluid">
			                            </div>              
			                            <div>
			                                <h4 class="fw-bold color-black" style="font-size: 1rem; height: 2rem">${hotel.title}</h4>
			                                <div class="mb-3">
			<%-- 							    	<div class="star_rating mb-4">
										        	<c:forEach begin="1" end="5" var="i">
										            	<span class="star ${i <= hotel.star_avg ? 'on' : ''}" value="${i}"></span>
										            </c:forEach>
										            <span style="color:black; margin-left: 10px; color: gray">(${hotel.review_num})</span>
										         </div> --%>
			                                <p class="mb-1 color-black"><i class="fa-solid fa-phone col-sm-1"></i> ${hotel.tel}</p>
			                                <p class="color-black"><i class="fas fa-map-marker-alt col-sm-1"></i> ${hotel.address}</p>
			                                </div>
			                            </div>
			                        </a>                     
			                    </div> 
                ...
                <script>
				    document.addEventListener('DOMContentLoaded', function() {
				        var hotelContainer = document.getElementById('hotelContainer');
				        var hotels = document.querySelectorAll('.border.p-3.mb-3');
				        ...
				        function updatePage() {
				            // 현재 페이지에 표시할 요소 선택
				            var currentPageHotels = Array.from(hotels).slice(startIndex, endIndex);
				            ...
				            // 현재 페이지에 해당하는 요소 보이기
				            currentPageHotels.forEach(function(hotel) {
				                hotel.style.display = 'block';
				            });
								
				        	 // 총 페이지 수 계산
				            var totalPages = Math.ceil(hotels.length / itemsPerPage);
				            ...
				            document.getElementById('nextPageButton').disabled = endIndex >= hotels.length;
				            document.getElementById('lastPageButton').disabled = endIndex >= hotels.length;
				        }
				        ...
				        function prevPage() {
				            if (currentPage > 1) {
				                ...
				                endIndex = Math.min(currentPage * itemsPerPage, hotels.length);
				                updatePage();
				            }
				        }
...
<script src="<c:url value="/resources/js/tab.js?ver=1"/>"></script>
</body>
</html>